...
# Мутирующие и немутирующие методы массивов js
...
```array.splice()``` удаляет указаное количество элементов с указанной позиции
Все три метода возвращают удаленные элементы.

```array.sort``` – сортирует массив «на месте», а потом возвращает его
```array.reverse``` – «на месте» меняет порядок следования элементов на противоположный и возвращает изменённый массив

Немутирующие
-

```array.concat()``` возвращает массив, объединяя значения двух мссивов
```array.filter()``` создает новый массив из первоначального массива, который содержит только те элементы, которые соотвествуют заданному критерию
```array.slice()``` возвращает новый массив, указывается стартовое и окончательное(не включительно) значения индексов элементов, которые будут взяты
```array.map()``` вернет новый массив, значения которого будут обработаны указанной функцией
```array.indexOf()```, ```array.lastIndexOf()``` – ищет item, начиная с позиции pos, и возвращает его индекс или -1, если ничего не найдено
```array.includes()``` – возвращает true, если в массиве имеется элемент value, в противном случае false
```array.find()```, ```array.filter()``` – фильтрует элементы через функцию и отдаёт первое/все значения, при прохождении которых через функцию возвращается true
```array.findIndex()``` похож на find, но возвращает индекс вместо значения.
```array.forEach()``` – вызывает func для каждого элемента. Ничего не возвращает
```array.reduce()```, ```array.reduceRight()``` – вычисляет одно значение на основе всего массива, вызывая func для каждого элемента и передавая промежуточный результат между вызовами.
